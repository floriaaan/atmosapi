...
def getAllProbes():
    ...
    PROBES = []
    for i in range (1, len(ids) + 1):
        dbCursor.execute("SELECT id_utilisateur FROM SONDE WHERE id_sonde='%s'" % (i-1))
        user = json.dumps(dbCursor.fetchone())

        dbCursor.execute("SELECT sonde_pos_latitude FROM SONDE WHERE id_sonde='%s'" % (i-1))
        pos_x = json.dumps(dbCursor.fetchone())

        dbCursor.execute("SELECT sonde_pos_longitude FROM SONDE WHERE id_sonde='%s'" % (i-1))
        pos_y = json.dumps(dbCursor.fetchone())

        dbCursor.execute("SELECT sonde_nom FROM SONDE WHERE id_sonde='%s'" % (i-1))
        name = json.dumps(dbCursor.fetchone())

        dbCursor.execute("SELECT sonde_active FROM SONDE WHERE id_sonde='%s'" % (i-1))
        active = json.dumps(dbCursor.fetchone())
